...
export interface Selectable { poolId: number; out: bigint; reserveStx: bigint; partial?: boolean }
...
export function selectPool<T extends Selectable>(quotes: T[], _minOut: bigint): T {
  const live = quotes.filter((q) => q.out > 0n);
  if (live.length === 0) throw new Error("no pool can quote this amount");
  ...
  return eligible.sort((a, b) => {
    if (a.out !== b.out) return a.out > b.out ? -1 : 1;
    if (a.reserveStx !== b.reserveStx) return a.reserveStx > b.reserveStx ? -1 : 1;
