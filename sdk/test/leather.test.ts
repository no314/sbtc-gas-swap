...
const relayFetch = (async (url: string) => {
...
describe("SbtcGasSwapProvider (Leather SwapProviderService shape)", () => {
  ...
  test("quotes a gas-size swap: low tier, target amount is output minus the rebate", async () => {
    const [q] = await provider().getSwapQuotes({ baseAsset: "sbtc", baseProviderAsset: base, targetAsset: "stx", targetProviderAsset: target, baseAmount: { amount: 30_000n, decimals: 8, symbol: "sBTC" } });
    ...
    assert.equal(BigInt(q.providerQuoteData.quoteOutUstx) - 10_000n, q.targetAmount.amount);
  ...
  test("execution data is a sponsored deny-mode call with six args, three post-conditions and the relay list", async () => {
    const p = provider();
    const [q] = await p.getSwapQuotes({ baseAsset: "sbtc", baseProviderAsset: base, targetAsset: "stx", targetProviderAsset: target, baseAmount: { amount: 30_000n, decimals: 8, symbol: "sBTC" } });
    ...
    assert.equal(BigInt(d.minOutUstx), (BigInt(q.providerQuoteData.quoteOutUstx) * 9_000n) / 10_000n);
  });
