...
import { defaultTier, defaultSlippageBips, minOutFor, type RelayInfoLike } from "../src/defaults.js";
...
const relay: RelayInfoLike = { minTier: "low" };
...
describe("defaultTier", () => {
  test("amount thresholds: low up to 30,000 sats, mid between, high from 0.1 BTC", () => {
    ...
    assert.equal(defaultTier(30_000n, relay), "low");
    assert.equal(defaultTier(30_001n, relay), "mid");
    assert.equal(defaultTier(9_999_999n, relay), "mid");
    assert.equal(defaultTier(10_000_000n, relay), "high");
  });
  test("never below the relay's published minimum", () => {
    ...
    assert.equal(defaultTier(10_000_000n, { minTier: "low" }), "high");
  });
});

describe("defaultSlippageBips", () => {
  test("10 percent for gas-size swaps, 1 percent above 30,000 sats", () => {
    assert.equal(defaultSlippageBips(30_000n), 1_000n);
    assert.equal(defaultSlippageBips(30_001n), 100n);
  });
});

describe("minOutFor", () => {
  test("floors quote times (1 - slippage)", () => {
    assert.equal(minOutFor(91_144_400n, 1_000n), 82_029_960n);
    assert.equal(minOutFor(1_000_001n, 100n), 990_000n);
  });
  test("rejects slippage outside 0 to 9999 bips", () => {
    assert.throws(() => minOutFor(1n, 10_000n));
    assert.throws(() => minOutFor(1n, -1n));
  });
});

describe("buildSwapCall", () => {
  const base = { user: USER, amountSats: 30_000n, tier: "mid" as const, poolId: 1, quoteOut: 91_144_400n, slippageBips: 1_000n };
  test("produces a SIP-030 stx_callContract request with sponsored true, fee 0, deny mode and the exact post-conditions", async () => {
    ...
    assert.deepEqual(call.postConditions, expectedPostConditions({ user: USER, amount: 30_000n, tier: TIERS.mid, minOut: 82_029_960n, poolId: 1 }));
    assert.equal(call.minOut, 82_029_960n);
  ...
  test("DLMM uses the lte sBTC post-condition", () => {
    const call = buildSwapCall({ ...base, poolId: 3, amountSats: 1_000_000n, quoteOut: 3_000_000_000n, slippageBips: 100n });
    assert.equal((call.postConditions[0] as any).condition, "lte");
  });
  test("refuses when min-out would be below the tier, and says what to change", () => {
    assert.throws(() => buildSwapCall({ ...base, amountSats: 3n, quoteOut: 9_000n, tier: "mid" }), (e: unknown) => e instanceof SwapBuildError && e.code === "MIN_OUT_BELOW_TIER");
  });
  test("refuses bad bips, unknown pool, zero amount", () => {
    assert.throws(() => buildSwapCall({ ...base, integrator: USER, integratorBips: 101n }), (e: any) => e.code === "BAD_BIPS");
    assert.throws(() => buildSwapCall({ ...base, poolId: 7 }), (e: any) => e.code === "UNKNOWN_POOL");
    assert.throws(() => buildSwapCall({ ...base, amountSats: 0n }), (e: any) => e.code === "BAD_AMOUNT");
  });
});
