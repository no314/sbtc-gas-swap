...
describe("ChainClient reads", () => {
    test("XYK state", async () => {
        ...
        assert.equal(s.xBalance, 44730224n);
    ...
    test("Velar state including the fee fraction", async () => {
        ...
        assert.equal(s.reserveStx, 212869098374n);
    ...
    test("DLMM state: prices come from initial-price times the bin factor", async () => {
        ...
        assert.equal(s.bins[0].price, (19610n * 166460000n) / 100000000n);
        assert.equal(calls.filter((c) => c.includes("get-bin-balances")).length, 2);
    });
    test("sBTC balance, nonces, fee estimate, broadcast", async () => {
        const { client } = fakeNode();
        assert.equal(await client.getSbtcBalance("SP227DXSVTHZZKF3B9N38G5GGG03N1Z4V86ZC3JVH"), 123456n);
        ...
        assert.equal(b.txid, "ab".repeat(32));
    });
...
describe("quoteAllPools", () => {
    test("quotes every pool it can read and selects the best; a failed pool read is reported, not zeroed", async () => {
        ...
        const q = await quoteAllPools(client, { amountSats: 30000n, integratorBips: 0n });
        assert.equal(q.fees.net, 29850n);
        assert.equal(q.quotes.find((x) => x.poolId === 1)?.out, 90692795n); // net 29,850 sats through XYK
    ...
    test("DLMM wins for a large swap when it quotes best", async () => {
        const { client } = fakeNode();
        const q = await quoteAllPools(client, { amountSats: 100000000n, integratorBips: 0n });
        assert.equal(q.best.poolId, 3);
        assert.ok(q.best.out > 100000000n);
    });
});
