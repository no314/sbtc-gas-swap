...
const info = (url, minTier, mid = "5000") => ({
...
function fakeRelays(behaviour) {
    const f = (async (url, init) => {
        const u = new URL(url);
        const h = behaviour[u.origin];
        if (!h)
            return new Response("no such relay", { status: 502 });
...
describe("RelayClient", () => {
    ...
    test("submit falls through to the next relay on transient errors and stops on a final verdict", async () => {
        ...
        assert.equal(ok.relay, "https://b.example");
        const c2 = fakeRelays({
...
describe("verifyContract", () => {
    ...
    test("a failed source read is unavailable, not a mismatch", async () => {
        const client = new ChainClient({ fetch: (async () => new Response("nope", { status: 500 })), minSpacingMs: 0, retries: 0 });
    ...
    test("live source equal to the reviewed source verifies", async () => {
        const client = new ChainClient({ fetch: (async () => new Response(JSON.stringify({ source, publish_height: 1 }))), minSpacingMs: 0, retries: 0 });
        const v = await verifyContract(client);
        assert.equal(v.ok, true);
    });
});
//# sourceMappingURL=relays.test.js.map