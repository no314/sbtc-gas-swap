...
export // A fake node: answers call-read by function name with shape-correct Clarity values.
 function fakeNode(overrides = {}) {
    const calls = [];
    const ok = (cv) => ({ okay: true, result: Cl.serialize(Cl.ok(cv)) });
    const answers = {
        "xyk-pool-sbtc-stx-v-1-1/get-pool": () => ok(Cl.tuple({
            "x-balance": Cl.uint(44730224n), "y-balance": Cl.uint(136671025817n),
            "x-protocol-fee": Cl.uint(10), "x-provider-fee": Cl.uint(40), "pool-status": Cl.bool(true),
        })),
        "univ2-pool-v1_0_0-0070/get-pool": () => ok(Cl.tuple({ reserve0: Cl.uint(212869098374n), reserve1: Cl.uint(69663360n) })),
        ...
        "dlmm-core-v-1-1/get-bin-factors-by-step": () => ok(Cl.some(Cl.list(Array.from({ length: 1001 }, (_, i) => Cl.uint(i === 840 ? 166460000n : i === 841 ? 166710000n : 100000000n))))),
        "dlmm-pool-stx-sbtc-v-2-bps-15/get-bin-balances": () => ok(Cl.tuple({ "x-balance": Cl.uint(50000000000n), "y-balance": Cl.uint(0), "bin-shares": Cl.uint(1) })),
        "sbtc-token/get-balance": () => ok(Cl.uint(123456n)),
        ...overrides,
    };
    const fetchImpl = (async (url, init) => {
        ...
        if (m) {
            ...
            const a = answers[key];
            if (!a)
                return new Response(JSON.stringify({ okay: false, cause: `no fixture for ${key}` }), { status: 200 });
        ...
        if (url.includes("/v2/accounts/"))
            return new Response(JSON.stringify({ balance: "0x0", nonce: 5 }));
        if (url.includes("/v2/fees/transaction"))
            return new Response(JSON.stringify({ estimations: [{ fee: 1000 }, { fee: 5000 }, { fee: 9000 }] }));
        if (url.includes("/v2/transactions"))
            return new Response(JSON.stringify("0x" + "ab".repeat(32)));
