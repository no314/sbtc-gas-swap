...
async function main() {
    ...
    if (!key)
        throw new Error("DUST_USER_KEY is required");
    const relays = (process.env.RELAYS ?? "").split(",").map((s) => s.trim()).filter(Boolean);
    if (relays.length === 0)
        throw new Error("RELAYS is required (comma separated relay base URLs)");
    ...
    if (balance < amountSats)
        throw new Error("insufficient sBTC");
    const q = await quoteAllPools(chain, { amountSats, integratorBips: 0n });
    for (const x of q.quotes)
        console.log(`pool ${x.poolId}: out ${x.out} uSTX, fee ${x.feeSats} sats, impact ${x.impactBips} bips${x.reason ? `, ${x.reason}` : ""}`);
    for (const u of q.unavailable)
        console.log(`pool ${u.poolId}: unavailable (${u.error})`);
    console.log("best pool", q.best.poolId, "out", q.best.out.toString());
    const candidates = await relayClient.discover();
    for (const c of candidates)
        console.log("relay", c.url, c.info ? `minTier ${c.info.minTier} fees ${JSON.stringify(c.info.feeEstimate)}` : `unreachable: ${c.error}`);
    const minTier = candidates.flatMap((c) => (c.info ? [c.info.minTier] : []))[0] ?? "low";
    const tier = process.env.TIER ?? defaultTier(amountSats, { minTier });
    ...
    if (!r.ok) {
        for (const a of r.attempts)
            console.log("relay", a.relay, a.code, a.message, "->", explainRelayError(a.code).action);
        process.exit(3);
    }
    ...
    while (Date.now() < deadline) {
        ...
        if (res.status === 404) {
            console.log("pending (not indexed yet)");
            continue;
        }
...
//# sourceMappingURL=dust-swap.js.map